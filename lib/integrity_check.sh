...
##########################################
# 📦 Pacman File Integrity
##########################################
echo "🔹 Pacman integrity check (modified core files only):"
echo
{
    sudo pacman -Qkk 2>/dev/null |
    grep -E 'Size mismatch|SHA256 checksum mismatch' |
    grep -Ev '/etc/|/var/|/run/|/tmp/|/home/'
} || true
echo ""

##########################################
# 🛡️ arch-audit (Known CVEs)
##########################################
if command -v arch-audit >/dev/null 2>&1; then
    echo "🔹 Security audit (known vulnerabilities):"
    AUDIT_OUT="$(arch-audit 2>/dev/null)" || true
    if [[ -n "${AUDIT_OUT:-}" ]]; then
        echo "$AUDIT_OUT"
    ...
    echo "⚠️ arch-audit not installed — skipping CVE check."
fi
echo ""
...
if [[ "$BOOT_FS" == "vfat" ]]; then
    echo "⚠️ /boot is FAT32 (vfat). chmod/chown cannot be enforced on this filesystem."
    ...
    if [[ "$BOOT_PERM" -gt 700 || "$SEED_PERM" -gt 600 ]]; then
        echo "⚠️ /boot or random-seed file may be world-readable. Current permissions:"
